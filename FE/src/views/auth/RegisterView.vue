                ...
                <div class="col-lg-5 col-12">
                    <div id="auth-left">
                        ...
                        <form @submit="onSubmit">
                            ...
                            <!-- full name -->
                            <div class="form-group position-relative has-icon-left mb-3">
                                <input type="text" maxlength="100" class="form-control form-control-xl" placeholder="Full name" v-model="fullName.value.value" :class="{'is-invalid' : fullName.errorMessage.value}">
                                <div class="form-control-icon">
                                    <span class="ionicon ionicon-id-card-outline fs-3"></span>
                                </div>
                                <p class="invalid-feedback">{{ fullName.errorMessage.value }}</p>
                            </div>
                            <!-- email -->
                            <div class="form-group position-relative has-icon-left mb-3">
                                <input type="email" maxlength="100" class="form-control form-control-xl" placeholder="Email" v-model="email.value.value" :class="{'is-invalid' : email.errorMessage.value}">
                                <div class="form-control-icon">
                                    <span class="ionicon ionicon-mail-outline fs-3"></span>
                                </div>
                            ...
                            <!-- confirm password -->
                            <div class="form-group position-relative has-icon-left mb-3">
                                <input type="password" maxlength="128" class="form-control form-control-xl" placeholder="Confirm Password" v-model="confirmPassword.value.value" :class="{'is-invalid' : confirmPassword.errorMessage.value}">
                                <div class="form-control-icon">
                                    <span class="ionicon ionicon-shield-checkmark-outline fs-3"></span>
                                </div>
                                <p class="invalid-feedback">{{ confirmPassword.errorMessage.value }}</p>
                            </div>
                        ...
                        <div class="text-center mt-5 text-lg">
                            <p class="text-gray-600">Already have an account? <RouterLink :to="{name: 'auth.login'}" class="font-bold">Login</RouterLink>.</p>
                        </div>
                    </div>
                </div>
...
import { 
    alpha_spaces as alphaSpacesRule,
    alpha_num as alphaNumRule,
...
defineRule("alpha_spaces", alphaSpacesRule);
...
const fullName = useField<string>("Full name", "required|alpha_spaces|min:5")
const email = useField<string>("Email", "required|email|min:5")
const password = useField<string>("Password", "required")
const confirmPassword = useField<string>("Confirm Password", "required|confirmed:@Password")
...
const onSubmit = handleSubmit(async() => {
    ...
    const data = await register({
        ...
        confirmPassword: md5Hash(confirmPassword.value.value)
    })
